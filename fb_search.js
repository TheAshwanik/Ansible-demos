...
function processFeed(obj) {
    ...
    if (obj.data[0]) {
        if (obj.paging
            .next) fb_older = obj.paging
            .next;
        for (var i = 0; i < obj.data
            .length; i++) {
            o = obj.data[i];
            str = '';
            ...
            if (search_links && o.type == 'link') {
                str = '<span class="fb_link"><a target="_blank" href="' + o.link + '"><br />';
                if (!(typeof o.name == "undefined")) str += o.name;
                str += '<br/>'+o.link+'</a></span>';
                if (!(typeof o.description == "undefined")) str += '<br />'+o.description;
                if (!(typeof o.message == "undefined")) str += '<br />'+o.message;
            } else if (search_photos && o.type == 'photo') {
                str+=o.name+' : ';
                img ='<span class="fb_photo"><img src="http://graph.facebook.com/'+o.object_id+'/picture?type=normal"/></span>';
                ...
                msg = shortify(msg);
                str += '<a class="fb_message" target="_blank" href="' + o.link + '">' + img + msg + '</a>';
                ...
                str += '<a class="video_link" style="background: #fff url(' + o.picture + ');" target="_blank" href="' + o
                .link + '" onclick="this.removeChild(this.firstChild);this.innerHTML=\'<iframe width=400 height=265 src=' + url + '></iframe>\';return false;"><img src="http://static.ak.fbcdn.net/rsrc.php/v1/yr/r/XXVvDYAks_i.png"></a>';
                str += '<b>Video: </b>';
            ...
            if (str != '') { //if there is anything, create row and append string to it
                str = '<span class="from"><a href="http://www.facebook.com/profile.php?id=' + o.from
                .id + '" target="_blank"><img src="http://graph.facebook.com/' + o
                .from
                .id + '/picture?type=' + photo_size + '"></a><a class="name" href="http://www.facebook.com/profile.php?id=' + o
                .from
                .id + '" target="_blank">' + o
                .from
                .name + '</a></span>' + str;
                ...
                if (!(typeof o.application == "undefined")) {
                    if (o.application != null && o
                        .application
                        .name != 'Links' && o
                        .application
                        .name != 'Likes' && o
                        .application
                        .name != 'Photos') str += '<span class="via"> via ' + formatApp(o.application
                        .name) + '</span>';
                }
                str += '</div>';
                row.innerHTML = str;
        ...
        if (obj.data
            .length != fb_count) dgbi("older_holder").innerHTML = '<span id="message">No more results!</span>';
        dgbi('status').style
        .display = 'none';
...
function watch_scroll() {
    ...
    var total = (document.body
        .scrollHeight - wh);
    var remain = total - document.body
    .scrollTop;
...
function showOlder() {
    toggle(dgbi('status'));
    toggle(dgbi('status1'));
    addJS(fb_older + '&callback=processFeed');
}
